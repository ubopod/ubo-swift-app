...
//  `[b]…[/b]`, `[color=#xxx]…[/color]` etc., which Kivy's GUI client
//  paints in place. The Web UI strips them; here we honour them so iOS
//  / Watch get the same bold/coloured emphasis as the Pi screen.
//
//  Supported tags: [b], [i], [u], [color=#hex]. [size=N] and unknown
//  bracketed tags (e.g. [ref=...]) are stripped, not rendered as
//  literal `[…]`.
//

import SwiftUI

/// Convert markup-bearing text into a SwiftUI `Text` with bold / italic /
/// underline / colour applied per-segment. Returns a plain `Text(raw)`
/// fast path when the input has no `[` so unmarked strings stay free.
...
/// places that need a `String` (titles, accessibility labels, search).
...
private func parseMarkupSegments(_ raw: String) -> [MarkupSegment] {
    ...
    for match in matches {
        ...
        case "size":
            break // SwiftUI Text has no per-segment relative size; ignored.
        default:
            break
        }
        stack.append(top)
