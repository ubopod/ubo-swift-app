...
//  ubo Watch App
//
//  Captures PCM16 microphone samples on Apple Watch via AVAudioEngine and
//  streams them to the device as `AudioReportSampleAction`s. Watch parallel
//  of the iOS service — same wire format (PCM16 mono @ 16 kHz) so the Pi's
//  assistant pipeline receives identical frames regardless of which client
//  is talking.
//
...
final class MicCaptureService {
    ...
    func start() async throws {
        ...
        // watchOS does not support `.defaultToSpeaker` — Watch routes audio
        // through paired BT or the built-in speaker automatically. Bluetooth
        // is allowed so users can talk through AirPods etc.
